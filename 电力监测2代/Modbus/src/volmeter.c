...
void Write_Config(void)
{
	...
	memset(write_buf,0,30);					//清空缓存
	
	AT24Cxx_ReadBuf(0,1,&len,1);
//	printf("len=%d\r\n",len);
	if((len>0)&&(len<5))
	{
		AT24Cxx_ReadBuf(0, 30, (u8*)write_buf, len);	//从EEPROM中读取变比配置信息
		if(write_buf[0]!=0)
		{
			IB_flag=1;
			for(i=0;i<len;i++)
			{
				Ibianbi=Ibianbi*10+write_buf[i]-'0';
			}
			printf("Ibianbi1=%d\r\n",Ibianbi);
		}
	}
	...
	AT24Cxx_ReadBuf(0,2,&len,1);
	if((len>0)&&(len<5))
	{
		AT24Cxx_ReadBuf(0, 40, (u8*)write_buf, len);
		if(write_buf[0]!=0)
		{
			UB_flag=1;
			for(i=0;i<len;i++)
			{
				Ubianbi=Ubianbi*10+write_buf[i]-'0';
			}
			printf("Ubianbi1=%d\r\n",Ubianbi);
		}
	}	
	...
	AT24Cxx_ReadBuf(0,5,&len,1);
	if((len>0)&&(len<5))
	{
		AT24Cxx_ReadBuf(0, 50, (u8*)write_buf, len);	//从EEPROM中读取变比配置信息
		if(write_buf[0]!=0)
		{
			IB2_flag=1;
			for(i=0;i<len;i++)
			{
				Ibianbi2=Ibianbi2*10+write_buf[i]-'0';
			}
			printf("Ibianbi2=%d\r\n",Ibianbi2);
		}
	}
	...
	AT24Cxx_ReadBuf(0,6,&len,1);
	if((len>0)&&(len<5))
	{
		AT24Cxx_ReadBuf(0, 60, (u8*)write_buf, len);
		if(write_buf[0]!=0)
		{
			UB_flag=1;
			for(i=0;i<len;i++)
			{
				Ubianbi2=Ubianbi2*10+write_buf[i]-'0';
			}
			printf("Ubianbi2=%d\r\n",Ubianbi2);
		}
	}
	...
	if(IB_flag==1||UB_flag==1)			//有电压比或电流比变化
	{
		IB_flag=0;
		UB_flag=0;
		Write_Config_buff[0]=Ubianbi>>8;
		Write_Config_buff[1]=Ubianbi;
		Write_Config_buff[2]=Ibianbi>>8;
		Write_Config_buff[3]=Ibianbi;
		
		MODH_Send10H(SlaveAddr,WriteConfig,2,Write_Config_buff);
		delay_ms(100);
		if(rs485_recv_flag==1)
		{
			MODH_Read_10H();
	...
	if(IB2_flag==1||UB2_flag==1)			//有电压比或电流比变化
	{
		IB2_flag=0;
		UB2_flag=0;
		Write_Config_buff[0]=Ubianbi2>>8;
		Write_Config_buff[1]=Ubianbi2;
		Write_Config_buff[2]=Ibianbi2>>8;
		Write_Config_buff[3]=Ibianbi2;
		
		MODH_Send10H(SlaveAddr2,WriteConfig,2,Write_Config_buff);
		delay_ms(100);
		if(rs485_recv_flag==1)
		{
			MODH_Read_10H();
...
{
	MODH_Send03H(_addr,ReadConfig,8);
	delay_ms(100);
	if(rs485_recv_flag==1)
	{
		if(_addr==1)
...
{
	MODH_Send03H(_addr,ReadParameters,17);
	delay_ms(200);
	if(rs485_recv_flag==1)
	{
		if(_addr==1)
...
{
	MODH_Send03H(_addr,ReadElectricEnergy,4);
	delay_ms(100);
	if(rs485_recv_flag==1)
	{
		if(_addr==1)
...
/************************************************************************************
* 函数名称：Buf2Uint32
* 功能描述：将2字节数组(大端Big Endian次序，高字节在前)转换为32位整数
* 入口参数：_pBuf : 数组
* 出口参数：16位整数值
*************************************************************************************/
	...
	switch(Reg03H)
	{
		case ReadConfig:
			if(CRC16_Modbus(RS485_RX_BUF,19)==(RS485_RX_BUF[19]<<8|RS485_RX_BUF[20]))  //判断CRC
			{
				...
				UART5_Send_Data(RS485_RX_BUF,21);
			break;
		case ReadParameters:
  ...
  u8 *p;
	switch(Reg03H)
	{
		case ReadConfig:
			...
			{
				p=&RS485_RX_BUF[7]; 								//数据开始地址
				g_ELE_PAM2.U0 = Buf2Uint16(p); p+=2;							//电压量程
				g_ELE_PAM2.I0 = Buf2Uint16(p); p+=4; 								//电流量程
				g_ELE_PAM2.UBB = Buf2Uint16(p); p+=2; 								//电压变比
				g_ELE_PAM2.IBB = Buf2Uint16(p);									 //电流变比
			}
			else //printf("485");
				UART5_Send_Data(RS485_RX_BUF,21);
			break;
		case ReadParameters:
			...
			{
				p=&RS485_RX_BUF[3]; 								//数据开始地址
				g_ELE_PAM2.UA = Buf2Uint16(p); p+=2;							//A相电压
				g_ELE_PAM2.UB = Buf2Uint16(p); p+=2; 								//B相电压
				g_ELE_PAM2.UC = Buf2Uint16(p); p+=2; 								//C相电压
				g_ELE_PAM2.IA = Buf2Uint16(p);	p+=2;							 //A相电流
				g_ELE_PAM2.IB = Buf2Uint16(p);	p+=2;							 //B相电流
				g_ELE_PAM2.IC = Buf2Uint16(p);	p+=2;							 //C相电流
				g_ELE_PAM2.P = Buf2Uint16(p);	p+=2;							 //有功功率
				g_ELE_PAM2.Q = Buf2Uint16(p);	p+=2;							 //无功功率
				g_ELE_PAM2.S = Buf2Uint16(p);	p+=2;							 //视在功率
				g_ELE_PAM2.PF = Buf2Uint16(p);	p+=10;							 //功率因数
				g_ELE_PAM2.Pa = Buf2Uint16(p);	p+=2;							 //A相功率
				g_ELE_PAM2.Pb = Buf2Uint16(p);	p+=2;							 //B相功率
				g_ELE_PAM2.Pc = Buf2Uint16(p);							 			//C相功率
			}
			else printf("485");
				//USART1_Send_Data(RS485_RX_BUF,39);
			break;
