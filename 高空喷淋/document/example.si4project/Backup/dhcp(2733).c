...
void send_DHCP_DISCOVER(void)
{
  ...
  uint16 i=0;
  uint16 size = sizeof(EXTERN_DHCPBUF);
  //the host name modified
  uint8 host_name[12];
  //*((uint32*)DHCP_SIP)=0;
  //*((uint32*)DHCP_REAL_SIP)=0;
  
  memsetx((u8*)EXTERN_DHCPBUF,0,size);
...
uint8 check_DHCP_state(SOCKET s) 
{
	...
	else
	{
		...
		{
#ifdef DHCP_DEBUG	   
			printf("client to create the DHCPC_SOCK(%u)\r\n",SOCK_DHCP);
#endif 			
		}
	}
	...
	switch (dhcp_state)											// DHCP状态转换过程
	{
		...
		case STATE_DHCP_DISCOVER :
			...
			else check_DHCP_Timeout();								// 检查DHCP是否超时
		break;
		...
		case STATE_DHCP_REREQUEST :				
			...
			else check_DHCP_Timeout();								// 重置DHCP超时时间和重连次数
		break;
		default :
