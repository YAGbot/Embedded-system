			...
			*/		
			switch(getSn_SR(0))/* 获取socket0的状态 */
			{
				...
				case SOCK_ESTABLISHED:/* Socket处于连接建立状态 */ 							
						...
						socket_len = getSn_RX_RSR(0);						/* 读取W5500空闲接收缓存寄存器的值并赋给len，Sn_RX_RSR表示接收缓存中已接收和保存的数据大小 */ 
						if(socket_len > 0)
						{
							memset(socket_buf, 0, sizeof(socket_buf));
							sprintf((char *)socket_buf, "%s", "REC:");							 
							recv(0,socket_buf+4,socket_len);/* W5500接收来自客户端的数据，并通过SPI发送给MCU */																						
							send(0,socket_buf+4,socket_len);/* 接收到数据后再回给客户端，完成数据回环 */ 													
