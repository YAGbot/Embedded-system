...
u8 w5500_init(void)
{
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		/* IOå£é€Ÿåº¦ä¸?0MHz */
	GPIO_Init(W5500_CS_PORT, &GPIO_InitStructure);			/* åˆå§‹åŒ–ç«¯å?*/

	GPIO_InitStructure.GPIO_Pin = W5500_RST_PIN;					
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;		/* æŽ¨æŒ½è¾“å‡º */
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		/* IOå£é€Ÿåº¦ä¸?0MHz */
	GPIO_Init(W5500_RST_PORT, &GPIO_InitStructure);			/* åˆå§‹åŒ–ç«¯å?*/	

	
	return 0; 
}

void w5500_rest(void)
{
	W5500_RST_CLR;
	delay_ms(1);  
	W5500_RST_SET;
...
/**
  ...
  * @retval æ—?
  */
u8 w5500_net_init(u8 * mac, u8 * sub, u8 * gw, u8 * lip)
{
	u8 t[4];
	...
	setSUBR(sub);/*é…ç½®å­ç½‘æŽ©ç */
	setGAR(gw);/*é…ç½®é»˜è®¤ç½‘å…³*/
	setSIPR(lip);/*é…ç½®Ipåœ°å€*/

	/* Init. TX & RX Memory size of w5500 */
	sysinit(txsize, rxsize); /*åˆå§‹åŒ?ä¸ªsocket*/ 
	setRTR(2000);
	setRCR(3);
	...
	getSUBR(t);
	printf("SET SUB : %d.%d.%d.%d\r\n", t[0],t[1],t[2],t[3]);
	getGAR(t);
	printf("SET GW : %d.%d.%d.%d\r\n", t[0],t[1],t[2],t[3]);
...
{
	if (val == LOW) W5500_CS_CLR;
	else if (val == HIGH)W5500_CS_SET;
}
...
{
  wiz_write_buf(SHAR0, addr, 6);  						// ÏòÍ¨ÓÃ¼Ä´æÆ÷ÇøµÄSHAR0¼Ä´æÆ÷Ð´ÈëMACµØÖ·
}
