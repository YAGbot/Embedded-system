  ...
  switch(getSn_SR(s))
  {
    case SOCK_UDP:
      if ((len = getSn_RX_RSR(s)) > 0)
      {
        ...
        else dns_wait_time = DNS_RESPONSE_TIMEOUT;
      }
      else
      {
        dns_wait_time++;
        print("dns wait time=%d\r\n", dns_wait_time);
      }
    ...
    case SOCK_CLOSED:
		...
		print("dns socket init over\r\n");
		len = dns_makequery(0, name, BUFPUB, MAX_DNS_BUF_SIZE);
		print("dns make query %s\r\n", name);
		u8 dns[4] = {8, 8, 8, 8};
		sendto(s, BUFPUB, len, dns/*EXTERN_DNS_SERVERIP*/, IPPORT_DOMAIN);
      break;         
  }
