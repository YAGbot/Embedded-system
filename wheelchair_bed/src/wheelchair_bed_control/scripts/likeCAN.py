...
#def receive_func(DEF_DEV_INDEX):
...
#延时创建发送线程
  ...
  send = ubyte_array(0x00,0x1a,0x06,0x0F,0xFF,0x00,0x00,0x00)
  print(send)
  CAN_recieveData = tagCAN_DataFrame(0,1,0,0,8,0x00000001,send)

  print(CAN_recieveData)
  ret = canDLL.CAN_ChannelSend(dwDeviceHandle,0,byref(CAN_recieveData),1)
  print(ret)
  if ret != CAN_RESULT_OK:
  ...
  if (DEF_USE_CAN_NUM & 1) == 1:
    ...
    buff_1 = ubyte_arr()
    buff_2 = ubyte_arr()
    can_registerconfig = canDLL.CAN_ReadRegister(dwDeviceHandle,0,0x038,buff_1,4)
    canDLL.CAN_ReadRegister(dwDeviceHandle,0,0x03C,buff_2,4)
    if can_registerconfig != CAN_RESULT_OK:
      print('调用 CAN_ReadRegister 出错\r\n')
    print(list(buff_1)) 
    print(list(buff_2)) 
  time.sleep(1)

  #tagCAN_DataFrame
  #uTimeFlag--c_uint--时间标识,对接收帧有效,从 CAN 通道启动开始计时
  #nSendType--c_ubyte--发送帧类型,0-正常发送;1-单次发送;2-自发自收;3-单次自发自收
  #bRemoteFlag--c_ubyte--是否是远程帧,0 表示数据帧,1 表示远程帧
  #bExternFlag--c_ubyte--是否是扩展帧,0 表示标准帧,1 表示扩展帧
  #nDataLen--c_ubyte--数据长度(<=8),也就是 arryData 的长度
  ...
  recieve = ubyte_array(0,0,0,0,0,0,0,0)
  CAN_recieveData = tagCAN_DataFrame(0,0,0,0,8,0x0,recieve)
  ret = canDLL.CAN_ChannelReceive(dwDeviceHandle,0,byref(CAN_recieveData),1,200)
  print(ret)

  if ret <= 0:  
    print('调用 Receive 出错\r\n')
    err = tagCAN_ErrorInformation()
    errInfo = canDLL.CAN_GetErrorInfo(dwDeviceHandle,0,byref(err))
    if ( errInfo != CAN_RESULT_OK ):
      print("GetDeviceInfo error\n")
#      exit()
    print(err)
#    print("ErrorCode: %d\n" %(errInfo.uErrorCode))
#    print("PassiveErrData: %x\n" %(list(errInfo.PassiveErrData)))
#    print("ArLostErrData: %x\n" %(errInfo.ArLostErrData))
    ret = canDLL.CAN_ChannelReceive(dwDeviceHandle,1,byref(CAN_recieveData),1,200)
    time.sleep(1)
  else:
    print('调用 Receive 成功\r\n')
    print(CAN_recieveData.uID)
    print(CAN_recieveData.nDataLen)
    print(list(CAN_recieveData.arryData))
