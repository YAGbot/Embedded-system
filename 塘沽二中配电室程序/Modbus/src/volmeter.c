...
char EndStr[500];
u8 Temp_str[50],Smok_str[50],Water_str[50];
u8 Write_Clear_buff[16]={0};			//写10H多个寄存器保持值buf[],写入的值根据buf[]内容修改
u8 Temp_cmd[8]={0x04,0x03,0x00,0x00,0x00,0x02,0xC4,0x5E};		//温湿度
u8 Smok_cmd[8]={0x02,0x03,0x00,0x00,0x00,0x01,0x84,0x39};		//烟感  
u8 Water_cmd[8]={0x03,0x03,0x00,0x02,0x00,0x01,0x24,0x28};	//水浸 1正常  2报警


void Data_pack(void)
{
	Get_Data01();	//计算相关电参数
	sprintf(EndStr,"{\"DataList\":[{\"Ua\":\"%0.1f\",\"Ub\":\"%0.1f\",\"Uc\":\"%0.1f\",\"Ia\":\"%0.1f\",\"Ib\":\"%0.1f\",\"Ic\":\"%0.1f\",\"P\":\"%0.1f\",",g_pack1.Ua1,g_pack1.Ub1,g_pack1.Uc1,g_pack1.Ia1,g_pack1.Ib1,g_pack1.Ic1,g_pack1.P1);
	sprintf(EndStr,"%s%s%s%s}],",EndStr,Temp_str,Smok_str,Water_str);
	sprintf(EndStr,"%s\"DevID\":\"TGDL%04dEP-AC\"}",EndStr,1);
	gu900_printf("%s\r\n",EndStr);
	printf("EndStr=%s\r\n",EndStr);
}
...
void Read_Temp(void)
{
	u16 Temp,Humi;
	float t,t1;
	RS485_Send_Data(Temp_cmd, 8);
	delay_ms(100);
	if(USART2_RX_STA&0X8000)
	{
		if(CRC16_Modbus(RS485_RX_BUF,7)==(RS485_RX_BUF[7]<<8|RS485_RX_BUF[8]))  //判断CRC
		{
			Temp=RS485_RX_BUF[3]*256+RS485_RX_BUF[4];
			t=Temp*0.1;

			Humi=RS485_RX_BUF[5]*256+RS485_RX_BUF[6];
			t1=Humi*0.1;
	...
	{
		sprintf((char*)Temp_str,"\"Temp\":\"_\",\"Humi\":\"_\",");
...
{
	u16 Smok;
	RS485_Send_Data(Smok_cmd, 8);
	delay_ms(100);
	if(USART2_RX_STA&0X8000)
	{
		if(CRC16_Modbus(RS485_RX_BUF,5)==(RS485_RX_BUF[5]<<8|RS485_RX_BUF[6]))  //判断CRC
		{
			Smok=RS485_RX_BUF[3]*256+RS485_RX_BUF[4];
			sprintf((char*)Smok_str,"\"Smok\":\"%d\",",Smok);
			printf("Smok_str=%s",Smok_str);
		}
		USART2_RX_STA=0;
	}
	else
	{
		sprintf((char*)Smok_str,"\"Smok\":\"_\",");
		printf("sendcomandsmok\r\n");
	}
	delay_ms(50);
}
...
{
	u16 Water;
	RS485_Send_Data(Water_cmd, 8);
	delay_ms(100);
	if(USART2_RX_STA&0X8000)
	{
		if(CRC16_Modbus(RS485_RX_BUF,5)==(RS485_RX_BUF[5]<<8|RS485_RX_BUF[6]))  //判断CRC
		{
			Water=RS485_RX_BUF[3]*256+RS485_RX_BUF[4];
			sprintf((char*)Water_str,"\"Water\":\"%d\"",Water);
			printf("Water_str=%s",Water_str);
		}
		USART2_RX_STA=0;
	}
	else
	{
		sprintf((char*)Water_str,"\"Water\":\"_\"");
		printf("sendcomandWater\r\n");
	}
	delay_ms(50);
}
